...
TAMBOPATA = pd.read_csv('data/TAMBOPATA_output.csv')
dicharge_rid = {62269800591: 63.302}
...
TAMBOPATA = TAMBOPATA.merge(discharge_series, on='reach_id', how='left').dropna()

with open('data/inverted_discharge_params.pickle', 'rb') as f:
    params = pickle.load(f)
def inverse_power_law(y, a, b):
    return (y / a) ** (1 / b)
TAMBOPATA['corrected_discharge'] = inverse_power_law(TAMBOPATA['discharge_uncorrected'], *params)
TAMBOPATA['slope'] = TAMBOPATA['slope'] / 1000

guesswork = TAMBOPATA[['width', 'slope', 'corrected_discharge']].astype(float)
...
TAMBOPATA['XGB_depth'] = xgb_reg.predict(guesswork)
TAMBOPATA['XGB_depth'] = TAMBOPATA['XGB_depth'].clip(lower=0)
df = TAMBOPATA
...
df.to_csv('data/TAMBOPATA_output_corrected.csv', index=False)
