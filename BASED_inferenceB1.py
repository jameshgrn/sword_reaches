...
B1 = pd.read_csv('data/B1_output.csv')
dicharge_rid = {62269900581: 356.765, 62269900571: 350.499, 62269900561: 341.238, 62269900551: 334.612,
...
B1 = B1.merge(discharge_series, on='reach_id', how='left').dropna()

with open('data/inverted_discharge_params.pickle', 'rb') as f:
    params = pickle.load(f)
def inverse_power_law(y, a, b):
    return (y / a) ** (1 / b)
B1['corrected_discharge'] = inverse_power_law(B1['discharge_uncorrected'], *params)
B1['slope'] = B1['slope'] / 1000

guesswork = B1[['width', 'slope', 'corrected_discharge']].astype(float)
...
B1['XGB_depth'] = xgb_reg.predict(guesswork)
B1['XGB_depth'] = B1['XGB_depth'].clip(lower=0)
df = B1
...
choices = [
    ...
    (df['ridge1_elevation'] - (df['channel_elevation'] - df['XGB_depth'])) + (df['ridge2_elevation'] - (df['channel_elevation'] - df['XGB_depth'])) / 2  # If a_b >= 1.5
]
...
df = df[df['lambda'] > 0]
df.to_csv('data/B1_output_corrected.csv', index=False)
