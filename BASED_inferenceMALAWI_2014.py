...
MALAWI_2014 = pd.read_csv('data/MALAWI_2014_output.csv')
dicharge_rid = {12222000071: 63.302, 12222000061: 63.302,
...
MALAWI_2014 = MALAWI_2014.merge(discharge_series, on='reach_id', how='left').dropna()

with open('data/inverted_discharge_params.pickle', 'rb') as f:
    params = pickle.load(f)
def inverse_power_law(y, a, b):
    return (y / a) ** (1 / b)
MALAWI_2014['corrected_discharge'] = inverse_power_law(MALAWI_2014['discharge_uncorrected'], *params)
MALAWI_2014['slope'] = MALAWI_2014['slope'] / 1000

guesswork = MALAWI_2014[['width', 'slope', 'corrected_discharge']].astype(float)
...
MALAWI_2014['XGB_depth'] = xgb_reg.predict(guesswork)
MALAWI_2014['XGB_depth'] = MALAWI_2014['XGB_depth'].clip(lower=0)
df = MALAWI_2014
...
df.to_csv('data/MALAWI_2014_output_corrected.csv', index=False)
