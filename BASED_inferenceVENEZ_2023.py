...
VENEZ_2023 = pd.read_csv('data/VENEZ_2023_output.csv')
dicharge_rid = {61302000121: 432.189, 61302000111: 444.248, 61302000101: 462.398,
...
VENEZ_2023 = VENEZ_2023.merge(discharge_series, on='reach_id', how='left').dropna()

with open('data/inverted_discharge_params.pickle', 'rb') as f:
    params = pickle.load(f)
def inverse_power_law(y, a, b):
    return (y / a) ** (1 / b)
VENEZ_2023['corrected_discharge'] = inverse_power_law(VENEZ_2023['discharge_uncorrected'], *params)
VENEZ_2023['slope'] = VENEZ_2023['slope'] / 1000

guesswork = VENEZ_2023[['width', 'slope', 'corrected_discharge']].astype(float)
...
VENEZ_2023['XGB_depth'] = xgb_reg.predict(guesswork)
VENEZ_2023['XGB_depth'] = VENEZ_2023['XGB_depth'].clip(lower=0)
df = VENEZ_2023
...
df.to_csv('data/VENEZ_2023_output_corrected.csv', index=False)
