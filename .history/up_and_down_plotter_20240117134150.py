...
# Loading the data
out_df = pd.read_csv('VENEZ_outputFULL3.csv').dropna()


# Load the data
reach_ids = out_df['reach_id'].tolist()  # Assuming REACHD_IDS is the column in your CSV containing the reach_ids you
# are
# interested in.
reach_ids_str = ', '.join(map(str, reach_ids))

sql_sword_data = f'''
SELECT "reach_id", "slope"
FROM "sword_reachesv14"
WHERE "reach_id" IN ({reach_ids_str})
'''

sword_data_gdf = pd.read_sql(sql_sword_data, engine)
df = pd.merge(out_df, sword_data_gdf, left_on='reach_id', right_on='reach_id', how='left')
