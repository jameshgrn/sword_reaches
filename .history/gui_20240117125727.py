...
def maybe_loads_from_wkb(geometry):
    if isinstance(geometry, str) or isinstance(geometry, bytes):
        return wkb.loads(geometry, hex=True)
    return geometry


# Load the data
sword_data_gdf = gpd.read_parquet('all_elevations_gdf_AFR.parquet', crs='EPSG:4326')
...
def compute_attributes(df, labeled_points):
    df.set_index('along_track_distance', inplace=True, drop=False)
...
def compute_distance_to_river_center(df, labeled_points):
    if 'channel' in labeled_points and labeled_points['channel']:
        ...
        df['dist_to_river_center'] = (df['along_track_distance'] - channel_point_x).abs()
    else:
...
def label_cross_section(df):
    ...
    ax.plot(df['along_track_distance'], df['elevation'], '-o', markersize=2)
...
for idx in range(start_idx, len(cross_sections), n):
    ...
    df['geometry'] = df['geometry'].apply(maybe_loads_from_wkb)
