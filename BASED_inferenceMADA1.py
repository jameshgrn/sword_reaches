...
MADA1 = pd.read_csv('data/MADA1_output.csv')
dicharge_rid = {18192000531: 38.858
                }
...
MADA1 = MADA1.merge(discharge_series, on='reach_id', how='left').dropna()

with open('data/inverted_discharge_params.pickle', 'rb') as f:
    params = pickle.load(f)
def inverse_power_law(y, a, b):
    return (y / a) ** (1 / b)
MADA1['corrected_discharge'] = inverse_power_law(MADA1['discharge_uncorrected'], *params)

guesswork = MADA1[['width', 'slope', 'corrected_discharge']].astype(float)
...
MADA1['XGB_depth'] = xgb_reg.predict(guesswork)
MADA1['XGB_depth'] = MADA1['XGB_depth'].clip(lower=0)
df = MADA1
...
df.to_csv('data/MADA1_output_corrected.csv', index=False)

df_est = binscatter(x='dist_out', y='lambda', data=df, ci=(1,1), noplot=True)
