...
RIOPIRAI = pd.read_csv('data/RIOPIRAI_output.csv')
dicharge_rid = {62269800631: 9.104,
                62269800621: 11.362,
                62269800611: 15.562,
                62269800601: 22.907,
                62269800591: 26.64,}
...
RIOPIRAI = RIOPIRAI.merge(discharge_series, on='reach_id', how='left').dropna()

with open('data/inverted_discharge_params.pickle', 'rb') as f:
    params = pickle.load(f)
def inverse_power_law(y, a, b):
    return (y / a) ** (1 / b)
RIOPIRAI['corrected_discharge'] = inverse_power_law(RIOPIRAI['discharge_uncorrected'], *params)
RIOPIRAI['slope'] = RIOPIRAI['slope'] / 1000
guesswork = RIOPIRAI[['width', 'slope', 'corrected_discharge']].astype(float)
...
RIOPIRAI['XGB_depth'] = xgb_reg.predict(guesswork)
RIOPIRAI['XGB_depth'] = RIOPIRAI['XGB_depth'].clip(lower=0)
df = RIOPIRAI
...
df.to_csv('data/RIOPIRAI_output_corrected.csv', index=False)
